...
import { CIFRADO_PUZZLES } from "../../../src/data/puzzles/cifrado/puzzles";
import { CifradoPuzzle } from "../../../src/data/puzzles/cifrado/types";
...
function PuzzleCard({ puzzle }: { puzzle: CifradoPuzzle }) {
  const diff = DIFF_CONFIG[puzzle.difficulty];
  return (
    <Pressable
      style={({ pressed }) => [styles.card, pressed && styles.cardPressed]}
      onPress={() => router.push(`/puzzles/cifrado/${puzzle.id}` as never)}
    ...
    >
      ...
      <View style={styles.cardFooter}>
        <Text style={styles.cardMeta}>🔐 {puzzle.cipherName}</Text>
...
export default function CifradoListScreen() {
  return (
    <SafeAreaView style={styles.screen}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <Text style={styles.title}>Descifrar el mensaje</Text>
        <Text style={styles.subtitle}>
          Usá la clave del cifrado para decodificar el mensaje oculto y revelar la verdad.
        </Text>
        <View style={styles.list}>
          {CIFRADO_PUZZLES.map((p) => (
