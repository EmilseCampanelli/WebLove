...
import { ESCAPE_PUZZLES } from "../../../src/data/puzzles/escape/puzzles";
import { EscapePuzzle } from "../../../src/data/puzzles/escape/types";
...
function PuzzleCard({ puzzle }: { puzzle: EscapePuzzle }) {
  const diff = DIFF_CONFIG[puzzle.difficulty];
  return (
    <Pressable
      style={({ pressed }) => [styles.card, pressed && styles.cardPressed]}
      onPress={() => router.push(`/puzzles/escape/${puzzle.id}` as never)}
    ...
    >
      ...
      <View style={styles.cardFooter}>
        <Text style={styles.cardMeta}>📍 {puzzle.location} · {puzzle.steps.length} pasos</Text>
...
export default function EscapeListScreen() {
  return (
    <SafeAreaView style={styles.screen}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        <Text style={styles.eyebrow}>ESCAPE</Text>
        <Text style={styles.title}>Escapar de la escena</Text>
        <Text style={styles.subtitle}>
          Resolvé cada puzle en secuencia para escapar de la escena del crimen antes de que sea tarde.
        </Text>
        <View style={styles.list}>
          {ESCAPE_PUZZLES.map((p) => (
