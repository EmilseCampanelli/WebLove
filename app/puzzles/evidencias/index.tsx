...
import { EVIDENCIAS_PUZZLES } from "../../../src/data/puzzles/evidencias/puzzles";
import { EvidenciasPuzzle } from "../../../src/data/puzzles/evidencias/types";
...
function PuzzleCard({ puzzle }: { puzzle: EvidenciasPuzzle }) {
  const diff = DIFF_CONFIG[puzzle.difficulty];
  return (
    <Pressable
      style={({ pressed }) => [styles.card, pressed && styles.cardPressed]}
      onPress={() => router.push(`/puzzles/evidencias/${puzzle.id}` as never)}
    ...
    >
      ...
      <View style={styles.cardFooter}>
        <Text style={styles.cardMeta}>🃏 {puzzle.cards.length} evidencias · {puzzle.suspects.length} sospechosos</Text>
  ...
  return (
    <SafeAreaView style={styles.screen}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <Text style={styles.title}>Clasificar las pruebas</Text>
        <Text style={styles.subtitle}>
          Analizá cada evidencia, determiná si es relevante, engañosa o irrelevante, y luego acusá al culpable.
        </Text>
        <View style={styles.list}>
          {EVIDENCIAS_PUZZLES.map((p) => (
