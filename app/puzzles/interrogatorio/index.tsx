...
import { INTERROGATORIO_PUZZLES } from "../../../src/data/puzzles/interrogatorio/puzzles";
import { InterrogatorioPuzzle } from "../../../src/data/puzzles/interrogatorio/types";
...
function PuzzleCard({ puzzle }: { puzzle: InterrogatorioPuzzle }) {
  const diff = DIFF_CONFIG[puzzle.difficulty];
  return (
    <Pressable
      style={({ pressed }) => [styles.card, pressed && styles.cardPressed]}
      onPress={() => router.push(`/puzzles/interrogatorio/${puzzle.id}` as never)}
    ...
    >
      ...
      <View style={styles.cardFooter}>
        <Text style={styles.cardMeta}>💬 {puzzle.suspects.length} sospechosos · {puzzle.totalCredits} créditos</Text>
...
export default function InterrogatorioListScreen() {
  return (
    <SafeAreaView style={styles.screen}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <Text style={styles.title}>Interrogá a los sospechosos</Text>
        <Text style={styles.subtitle}>
          Usá tus créditos con cuidado. Cada pregunta cuesta. Acusá al culpable con las pruebas correctas.
        </Text>
        <View style={styles.list}>
          {INTERROGATORIO_PUZZLES.map((p) => (
...
const styles = StyleSheet.create({
  ...
  cardMeta: { fontFamily: "Manrope_400Regular", fontSize: 11, color: Colors.textTertiary, flex: 1 },
