...
import { DETECTIVE_PUZZLES } from "../../../src/data/puzzles/detective/puzzles";
import { DetectivePuzzle } from "../../../src/data/puzzles/detective/types";
...
function PuzzleCard({ puzzle }: { puzzle: DetectivePuzzle }) {
  const diff = DIFF_CONFIG[puzzle.difficulty];
  return (
    <Pressable
      style={({ pressed }) => [styles.card, pressed && styles.cardPressed]}
      onPress={() => router.push(`/puzzles/detective/${puzzle.id}` as never)}
    ...
    >
      ...
      <View style={styles.cardFooter}>
        <Text style={styles.cardMeta}>🗂️ {puzzle.suspects.length} sospechosos</Text>
...
export default function DetectiveListScreen() {
  return (
    <SafeAreaView style={styles.screen}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        <Text style={styles.eyebrow}>CASOS POLICIALES</Text>
        <Text style={styles.title}>Leé el expediente</Text>
        <Text style={styles.subtitle}>
          Analizá el expediente, marcá las pistas clave y acusá al culpable.
        </Text>
        <View style={styles.list}>
          {DETECTIVE_PUZZLES.map((p) => (
